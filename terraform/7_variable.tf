...
variable "ecs-alb-tg" {
  type    = string
  default = "ce7-ty-ecs-target-group"
}

#  ECR Repository
variable "ecr_repo" {
  type    = string
  default = "ce7-ty-ecr_repo"
}

# ECS Task Definition
variable "ecs_task_def" {
  type    = string
  default = "ce7-ty-task_def"
}

# ECS Port name
variable "ecs_port_name" {
  type    = string
  default = "ty-ecs-8080-tcp"
}

# ECS log group name
variable "ecs_log_group" {
  type    = string
  default = "/ecs/tyloggroup"
}

# ECS cluster name
variable "ecs_cluster_name" {
  type    = string
  default = "ce7-ty-ecs-cluster"
}

# ECS service name
variable "ecs_service_name" {
  type    = string
  default = "ce7-ty-ecs-service"
}